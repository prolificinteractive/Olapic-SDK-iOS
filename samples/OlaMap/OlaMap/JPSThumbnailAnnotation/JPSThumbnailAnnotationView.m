...
@property (nonatomic, strong) OlapicAsyncImageView *asyncImage;
...
-(void)setupLoader{
    _loading = [[UIActivityIndicatorView alloc] initWithFrame:CGRectMake(15, 12, 20, 20)];
...
-(void)setupAsyncImage{
    _asyncImage = [[OlapicAsyncImageView alloc] initWithMedia:_media callback:^(OlapicAsyncImageView *image){
        // ....
    } andFrame:CGRectMake(0, 0, 50.0f, 47.0f)];
    [_imageView addSubview:_asyncImage];
    [_asyncImage download];
}
...
- (void)updateWithThumbnail:(JPSThumbnail *)thumbnail {
    ...
    self.media = thumbnail.media;
    if(!self.asyncImage){
        [self setupAsyncImage];
    }
...
- (void)didTapDisclosureButton {
    if (self.disclosureBlock){
        self.disclosureBlock(self);
        [self performSelector:@selector(shrink) withObject:nil afterDelay:0.40];
    }
}
